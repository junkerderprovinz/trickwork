...
const COG =
  'M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 0 1-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 0 1 .947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 0 1 2.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 0 1 2.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 0 1 .947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 0 1-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 0 1-2.287-.947zM10 13a3 3 0 1 0 0-6 3 3 0 0 0 0 6z'

// An 8-tooth cog for Settings, as in BombVault and KnightLoader.
export function iconAppearance(): string {
  return svg(`<path fill-rule="evenodd" clip-rule="evenodd" fill="currentColor" d="${COG}" />`)
}

// A settings tab's glyph, filled and cropped to its ink so the three tabs'
// marks come out the same size.
function tabSvg(d: string, viewBox: string): string {
  return `<svg width="20" height="20" viewBox="${viewBox}" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" clip-rule="evenodd" d="${d}" /></svg>`
}

// The Settings cog, for the General tab.
export function iconGeneral(): string {
  return tabSvg(COG, '2 2 16 16')
}

// Theme, shape and colours, as in ArrowLoop. Streamline Core Solid,
// interface-essential/color-palette.svg (CC BY 4.0, https://streamlinehq.com).
...
// The other ways to get the app. Streamline Core Solid,
// computer-devices/computer-pc-desktop.svg (CC BY 4.0).
